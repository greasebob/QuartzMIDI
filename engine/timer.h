...
// Calibration settings are parameters, not statics read from the config:
// static initialisation runs before config.json is loaded.
...
static double __measure_tsc_freq_once(double measureTimeSec, int coreIndex)
{
    ...
    // Warm-up read; the value is unused.
...
// CPU frequency in Hz, or 0.0 if calibration failed.
