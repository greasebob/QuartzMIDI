...
namespace midi {
    ...
    void HotkeySettings::validate() const {
        ...
        // These may be empty (unbound); the three above are sent as keystrokes and are required.
    ...
    void PlaybackSettings::validate() const {
        if (velocityModifier != "alt" && velocityModifier != "ctrl" && velocityModifier != "shift") {
            // A character key would also trigger a note; velocity needs a modifier.
    ...
    // Every field is optional; missing ones keep their defaults.
    ...
    void from_json(const json& j, PlaybackSettings& p) {
        ...
        // Optional; older configs omit it.
    ...
    void Config::setDefaults() {
        ...
        // WootingAnalogSettings' member defaults match upstream.
        wooting = {};
        ...
        // FULL: lower octaves, then LIMITED, then higher octaves
        key_mappings["FULL"] = {
