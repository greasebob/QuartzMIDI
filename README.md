...
`QuartzMIDI-v1.0.2.zip` on the [release page](https://github.com/greasebob/QuartzMIDI/releases) includes both, as does `QuartzMIDI-v1.0.2.exe`, a single file that keeps them and its settings in `%APPDATA%\QuartzMIDI`. `QuartzMIDI-v1.0.2-no-addons.zip` leaves them out; add one by extracting its zip into the app's `addons` folder.
